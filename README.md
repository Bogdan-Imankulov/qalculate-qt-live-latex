...
> [!NOTE]
> This MSYS2 gnuplot build defaults to the `qt` terminal, which spawns a `gnuplot_qt.exe` helper that never finishes initializing when launched this way (piped from qalculate-qt, no visible console) - the plot window stays blank after a long wait. `src/main.cpp` sets `GNUTERM=windows` before launching, which makes gnuplot use its native, dependency-free Win32 terminal instead (opens near-instantly).
